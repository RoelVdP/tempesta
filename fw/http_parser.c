...
#define IS_CSTR_BAD(r)		(r < 0 && r != CSTR_POSTPONE)
/**
...
#define parse_int_list(data, len, acc)					\
...
__FSM_STATE(st_curr) {							\
	...
	default:							\
		...
		if (IS_CSTR_BAD(process_trailer_hdr(msg,		\
						    &parser->hdr, id)))	\
			TFW_PARSER_BLOCK(st_curr);			\
...
__FSM_STATE(st_curr) {							\
	...
	default:							\
		...
		if (IS_CSTR_BAD(process_trailer_hdr(msg, &parser->hdr,	\
						    TFW_HTTP_HDR_RAW)))	\
			TFW_PARSER_BLOCK(st_curr);			\
		parser->_i_st = &&RGen_EoL;				\
